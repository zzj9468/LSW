...
router.get('/:aname/:apwd',(req,res)=>{
    ...
    var sql='SELECT aid FROM lsw_admin WHERE aname=? AND apwd=PASSWORD(?)'
...
router.get('/setting',(req,res)=>{
    var sql='SELECT * FROM lsw_settings';
...
router.put('/setting',(req,res)=>{
    var data=req.body;
    var sql='UPDATE lsw_settings SET appname=?,apiUrl=?,adminUrl=?,appUrl=?,icp=?,copyright=?';
...
router.patch('/',(req,res)=>{
    ...
    //如果查询到,在修改密码
    pool.query('select aid from lsw_admin where aname=? and apwd=PASSWORD(?)',[data.aname,data.oldPwd],(err,result)=>{
        if(err)throw err;
        // console.log(data);
        if(result.length==0){
        ...
        var sql='UPDATE lsw_admin SET apwd=PASSWORD(?) WHERE aname=?';
