$(function(){
   ...
   //获得焦点的验证
    ...
    var span1=span_true($('.tel'));
    var span2=span_true($('.uname'));
    var span3=span_true($('.email'));
    var span4=span_true($('.upwd'));
    var span5=span_true($('.reUpwd'));
    var span6=span_true($('.code'));
    function span_true(span){
        return span.next().html()=='通过';
    }
    var span=span1&&span2&&span3&&span4&&span5&&span6;
    console.log(span);
    if(span&&$(':input[type=checkbox]').prop('checked')==true){
        var uname=$('.uname').val();
        var phone=$('.tel').val();
        var email=$('.email').val();
        var upwd=$('.upwd').val();
    ...
    }else if($('.register_a').prop('actived')){
        location.href=`login.html`;
    }else{ 
                alert(`信息填写失败`);
               $('register').prop('disabled');
